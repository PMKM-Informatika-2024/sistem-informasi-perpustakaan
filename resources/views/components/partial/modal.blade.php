...
<div x-cloak x-data="{ open: false, modal: '{{ $name }}', data: {}, type: '' }" x-show="open" x-on:open-modal.window="open = ($event.detail.modal === modal); data = ($event.detail.data || {}); type = ($event.detail.type || '');" x-on:close-modal.window="open = false"
  x-on:keyup.escape.window="open = false" class="fixed left-0 right-0 top-0 z-50 flex h-[calc(100%-1rem)] max-h-full w-full items-center justify-center overflow-y-auto overflow-x-hidden bg-black/30 backdrop-blur md:inset-0">
